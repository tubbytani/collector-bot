...
int counter=2;
int final_array[]={1,2,3,4,0,15,14,13,7,6,0,15};
int new_array[]=  {1,2,3,4,5,15,0 ,0 ,6,5,0,0 };
...
void wall()
{
	...
	/*while(center_sensor<=4||left_sensor<=4||right_sensor<=4)//while ( ((lvalue>=5) || (rvalue>=5)) && (center_sensor<=4))
	{
		forward();
		_delay_ms(15);
		rvalue= ADC_Conversion(5);
...
void dash() // function to cross the zig zag path of the arena
{
	...
	right_sensor = ADC_Conversion(3);
	center_sensor = ADC_Conversion(2);
	while ((left_sensor >=5) || (right_sensor>=5) || (center_sensor>=5)) // check for black line near the arena
	{
		forward();
		_delay_ms(30);
		...
		if ((center_sensor <=4) && (left_sensor <=4)) // if the black line is on the right
		{
			while ((center_sensor <=4) && (left_sensor<=4))
			{
				soft_right(); // slight right is taken to make the bot follow the correct path
				_delay_ms(100);
				stop();
				left_sensor = ADC_Conversion(1);
				right_sensor = ADC_Conversion(3);
		...
		if ((center_sensor <=4) && (right_sensor <=4)) // if the black line is on the left
		{
			while ((center_sensor <=4) && (right_sensor<=4))
			{
				soft_left(); // slight left is taken to make the bot follow the correct path
				_delay_ms(100);
				stop();
				left_sensor = ADC_Conversion(1);
				right_sensor = ADC_Conversion(3);
...
{ if ( ((final_array[i]==5)&&((final_array[i+1])==15))||((final_array[i]==15)&&((final_array[i+1])==5))) // nodes near wall to be crossed
	{
		...
		wall(); // function to cross wall called
		i++;
		fwd();
		counter--;
		boom(); 
	}
	if ( ((final_array[i]==7)&&((final_array[i+1])==13))||((final_array[i]==13)&&((final_array[i+1])==7))) // check for nodes near zig zag
	{  
		...
		fwd();
		counter--;
		dash(); // funtion  to call zig zag called
		boom();	
	}
	return;
}
void fwd(void) // function for straight forward movement of the bot
{
	...
	while ((left_sensor >=5) || (right_sensor >=5) || (center_sensor >=5))
	{
		...
		if ((center_sensor <5) && (left_sensor <5)) // to check left deviation of the bot
		{
			while ((center_sensor <5) && (left_sensor <5))
			{
				right();// to bring it back to the correct path
				left_sensor = ADC_Conversion(1);
				right_sensor = ADC_Conversion(3);
				center_sensor = ADC_Conversion(2);
			}
...
right_sensor = ADC_Conversion(3);
center_sensor = ADC_Conversion(2);
		if ((center_sensor <5) && (right_sensor <5)) // to check right deviation of the bot
		{
			while ((center_sensor <5) && (right_sensor<5))
			{
				left();// to bring it back to the correct path	
				left_sensor = ADC_Conversion(1);
				right_sensor = ADC_Conversion(3);
				center_sensor = ADC_Conversion(2);
			}
		...
		right_sensor = ADC_Conversion(3);
		center_sensor = ADC_Conversion(2);
		while ((left_sensor >=5) && (right_sensor >=5) && (center_sensor >=5)) // node encountered
		{  
			counter++;
			forward();
			_delay_ms(5);
			...
			{   forward();
				...
				_delay_ms(615);
				stop();
				if(right_sensor>=5||center_sensor>=5) //left turn encountered
			{
				fwd();
			}
			else
			...
			stop();
			//return;
			 if ( ((final_array[i]==5)&&((final_array[i+1])==15))||((final_array[i]==15)&&((final_array[i+1])==5))) // check for wall
			{   forward();
				_delay_ms(100);
				stop();
				zap();
...
{
	if ((counter==9)||(counter==13)|| (counter==11)|| (counter==7)) // if the node is either of these 
	{
		left();
		_delay_ms(600);
		stop();
		...
		right_sensor = ADC_Conversion(3);
		center_sensor = ADC_Conversion(2);
		if((left_sensor>=5)||(right_sensor>=5)||(center_sensor>=5)) // if black line is detected
		{
			counter++;
			fwd_wls();
		}
...
void bam(void) // function to govern the movement by calling other functions one by one
{
	...
	{  		 
		...
		//{if (rvalue<=200)
			...
			stop(); // destination reached
			flag=0;
			pick(); // material needed to be picked therefore function called
			j++;
			lcd_string("left");
			left();
			_delay_ms(800);
